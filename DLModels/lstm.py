...
class LSMTModel(nn.Module):
    ...
    def forward(self, x):
        ...
        score = torch.tanh(self.attn_hidden(lstm_out))  # (B, T, H)
        score = torch.matmul(score, self.context_vector)  # (B, T)
        attn_weights = torch.softmax(score, dim=1).unsqueeze(2)  # (B, T, 1)
...
# 10. LSTM Grid Search with Clean K-Fold CV + Final Test Evaluation
def run_lstm_kfold_grid_search():
    glove = load_glove()
    ...
    print("\n Classification Report: LSTM Model (Test Set)")
    ...
    plt.title("Confusion Matrix: LSTM Model (Test Set)")
